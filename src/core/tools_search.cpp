...
std::string LsTool::description() const {
  return R"json({"name":"ls","description":"List directory contents, sorted alphabetically; dirs suffixed with '/'. Truncated to 500 entries/100KB.","parameters":{"type":"object","properties":{"path":{"type":"string","description":"Dir to list, default cwd"},"limit":{"type":"number","description":"Max entries, default 500"}}}})json";
}

ToolResult LsTool::execute(const ToolArgs& args) const {
  ToolResult result;

  const std::string path = default_path(args);
  const int64_t limit =
      std::max<int64_t>(int_arg(args, "limit").value_or(500), 1);

  std::error_code ec;
  if (not std::filesystem::is_directory(path, ec) or ec) {
    result.error = "ls: not a directory: " + path;
    return result;
  }
  ...
  if (ec) {
    result.error = "ls: cannot list " + path + ": " + ec.message();
    return result;
  }
  ...
  for (; it != end; it.increment(ec)) {
    ...
    if (it->is_directory(entry_ec) and not entry_ec) name += "/";
    entries.push_back(std::move(name));
  }
  std::sort(entries.begin(), entries.end());

  const bool over_limit = static_cast<int64_t>(entries.size()) > limit;
  if (over_limit) entries.resize(static_cast<size_t>(limit));
  ...
  TruncatedOutput truncated =
      truncate_output(std::move(output), "ls", static_cast<size_t>(limit));

  result.ok = true;
  result.output = std::move(truncated.text);
  result.output += truncation_note(truncated);
  if (over_limit and not truncated.truncated) {
    result.output += "\n[entries capped at " + std::to_string(limit) + "]";
  }
  result.truncated = truncated.truncated or over_limit;
  result.overflow_path = std::move(truncated.overflow_path);
  return result;
