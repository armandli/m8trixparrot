...
vdb::IndexParams params_for(size_t dim, vdb::Metric metric = vdb::Metric::Cosine) {
  vdb::IndexParams params;
...
TEST_F(VectorIndexTest, DotProductMatchesAScalarReference) {
  ...
  for (const size_t dim : {1u, 3u, 4u, 7u, 16u, 17u, 33u, 128u, 383u}) {
    ...
    EXPECT_NEAR(vdb::dot_product(v[0].data(), v[1].data(), dim), want, 1e-3)
        << "dim " << dim;
  }
}
...
TEST_F(VectorIndexTest, L2SquaredMatchesAScalarReference) {
  for (const size_t dim : {1u, 3u, 4u, 7u, 16u, 17u, 33u, 128u, 383u}) {
    ...
    EXPECT_NEAR(vdb::l2_squared(v[0].data(), v[1].data(), dim), want, 1e-3)
        << "dim " << dim;
  }
}
...
TEST_F(VectorIndexTest, FindsExactNeighborsOnATinyGraph) {
  ...
  const std::vector<vdb::Neighbor> hits = index.search(query.data(), 2, {});
...
TEST_F(VectorIndexTest, RecallStaysAboveNinetyPercentAgainstAnExactScan) {
  ...
  vdb::VectorIndex index(params_for(dim));
  ...
  for (const std::vector<float>& query : queries) {
    const std::vector<vdb::Neighbor> approximate = index.search(query.data(), k, {});
    const std::vector<vdb::Neighbor> exact = index.scan(query.data(), k, {});
    std::set<uint32_t> truth;
    for (const vdb::Neighbor& n : exact) truth.insert(n.label);
    size_t hit = 0;
    for (const vdb::Neighbor& n : approximate) hit += truth.count(n.label);
...
TEST_F(VectorIndexTest, ScanReturnsResultsNearestFirst) {
  ...
  const std::vector<vdb::Neighbor> hits = index.scan(data[0].data(), 10, {});
...
TEST_F(VectorIndexTest, APredicateKeepsRejectedLabelsOutOfBothPaths) {
  ...
  const vdb::LabelPredicate every_seventh = [](uint32_t label) {
    return label % 7 == 0;
  };
  for (const std::vector<vdb::Neighbor>& hits :
       {index.scan(data[1].data(), 10, every_seventh),
        index.search(data[1].data(), 10, every_seventh)}) {
    EXPECT_FALSE(hits.empty());
    for (const vdb::Neighbor& n : hits) EXPECT_EQ(n.label % 7, 0u);
  }
}
...
TEST_F(VectorIndexTest, SnapshotRoundTripsTheGraph) {
  ...
  vdb::VectorIndex restored(params_for(dim));
  ...
  for (const std::vector<float>& query : queries) {
    const std::vector<vdb::Neighbor> want = index.search(query.data(), 10, {});
    const std::vector<vdb::Neighbor> got = restored.search(query.data(), 10, {});
...
TEST_F(VectorIndexTest, ASnapshotCoveringAPrefixIsExtendedByLinking) {
  ...
  vdb::VectorIndex partial(params_for(dim));
  ...
  // snapshot was taken.
  const std::vector<vdb::Neighbor> hits =
...
TEST_F(VectorIndexTest, AnEmptyIndexSearchesToNothing) {
  vdb::VectorIndex index(params_for(8));
...
TEST_F(VectorIndexTest, L2AndInnerProductOrderTheirOwnNearest) {
  ...
  vdb::VectorIndex l2(params_for(2, vdb::Metric::L2));
  ...
  vdb::VectorIndex ip(params_for(2, vdb::Metric::InnerProduct));
