...
TEST_F(PythonTest, CapturesStdout) {
  const tools::ToolResult result = tools::PythonTool().execute(args({{"script", str("print('hello')")}}));
...
TEST_F(PythonTest, CapturesStderr) {
  const tools::ToolResult result = tools::PythonTool().execute(
...
TEST_F(PythonTest, MultiLineScriptRuns) {
  const tools::ToolResult result = tools::PythonTool().execute(args({{"script", str(R"(
x = 6
y = 7
print(x * y)
)")}}));
...
TEST_F(PythonTest, MissingScriptIsAnError) {
  const tools::ToolResult result = tools::PythonTool().execute(args({}));
...
TEST_F(PythonTest, EmptyScriptIsAnError) {
  const tools::ToolResult result = tools::PythonTool().execute(args({{"script", str("")}}));
...
TEST_F(PythonTest, WrongArgTypeReadsAsMissing) {
  const tools::ToolResult result = tools::PythonTool().execute(args({{"script", num(42)}}));
...
TEST_F(PythonTest, ConsecutiveCallsAreIsolated) {
  tools::PythonTool().execute(args({{"script", str("print('first')")}}));

  const tools::ToolResult result = tools::PythonTool().execute(args({{"script", str("print('second')")}}));
...
TEST_F(PythonTest, DescriptionPointsToPackageInstallNotSubprocess) {
  const std::string description = tools::PythonTool().description();
